...
namespace Bike_Damage_Dijkstra
{
        ...
        static double Shortest()
        {
            ...
            while (queue.Count > 0)
            {
                ...
                for (int i = 0; i < 8; i++)
                {
                    ...
                    var potentialDamage = current.Damage + Math.Abs(matrix[current.X, current.Y] - matrix[newRow, newCol]);

                    if (potentialDamage < damage[newRow, newCol])
                    {
                        damage[newRow, newCol] = potentialDamage;
                        queue.Enqueue(new Cell((short)newRow, (short)newCol, potentialDamage));
                    }
                }
            }
    ...
    public class Cell : IComparable
    {
        ...
        public Cell(short x, short y, double damage)
        {
            this.X = x;
            this.Y = y;
            this.Damage = damage;
        ...
        {
            return this.Damage.CompareTo((obj as Cell).Damage);
        }
    }
